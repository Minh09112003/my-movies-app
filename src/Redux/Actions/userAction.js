...
const deleteFavoriteMoviesAction = () => async (dispatch, getState) => {
    try {
        ...
        await userApi.deleteFavoritesMovies(
            tokenProtection(getState)
        )
        dispatch({
            type: userConstants.DELETE_FAVORITE_MOVIES_SUCCESS,
        })
        toast.success("Phim yêu thích đã được xóa")
    } catch (error) {
...
const deleteUsersAction = (id) => async (dispatch, getState) => {
    try {
        ...
        await userApi.deleteUserService(
            id,
            tokenProtection(getState)
        )
        dispatch({
...
const likeMovieAction = (movieId) => async (dispatch, getState) => {
    try {
        ...
        toast.success("Phim đã được thêm vào mục yêu thích")
